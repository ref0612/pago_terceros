...
const state = {
  ...
  filter:       'all',
  search:       '',
  hideEmpty:    false,   // ← nuevo: ocultar sin viajes
  expanded:     new Set(),
};
...
// Calcula start_date y end_date a partir de fecha base y período
// Ej: base=2026-03-19, days=3 → start=2026-03-17, end=2026-03-19
...
// ─── FETCH SERVICIOS con fecha real ──────────────────────────
// La API acepta start_date y end_date en formato DD/MM/YYYY
async function fetchAllServices(baseDate, days) {
  const { start, end } = dateRange(baseDate, days);

  // Convertir YYYY-MM-DD → DD/MM/YYYY para la API de Konnect
  const toApiDate = s => s.split('-').reverse().join('/');
  const startApi  = toApiDate(start);
  const endApi    = toApiDate(end);

  // Intentamos primero con start_date/end_date explícitos
  // Si la API no los soporta, caemos en date_range=days (últimos N días desde hoy)
...
function computeGlobalSummary() {
  let totalProd = 0, totalCom = 0, totalNeto = 0, totalSvc = 0;
  // Solo incluir empresarios que coincidan con filtros actuales
  const empresas = getFiltered();
  for (const emp of empresas) {
    for (const row of (state.services[emp.ownerCode] || [])) {
      totalProd += parseMoney(row[COL.produccion]);
      totalCom  += parseMoney(row[COL.comision]);
      totalNeto += parseMoney(row[COL.totalNeto]);
      totalSvc++;
  ...
  const { start, end } = dateRange(state.baseDate, state.days);
  const fmt = s => s.split('-').reverse().join('/');
  $('s-period').textContent = `${fmt(start)} → ${fmt(end)}`;
}
...
function getFiltered() {
  ...
  return state.empresarios.filter(emp => {
    // Búsqueda por nombre, código o RUT
    if (s) {
      const name = `${emp.firstName} ${emp.lastName}`.toLowerCase();
...
function renderCards() {
  ...
  // Actualizar summary según filtros activos
  computeGlobalSummary();
...
async function loadData() {
  ...
  // Mostrar rango de fechas en el loader
  const { start, end } = dateRange(state.baseDate, state.days);
  const fmt = s => s.split('-').reverse().join('/');
  setLoading(`Cargando ${state.days}d: ${fmt(start)} → ${fmt(end)}…`);
  ...
  try {
    ...
    $('loading-msg').textContent = `Cargando servicios ${fmt(start)} → ${fmt(end)}…`;
    const allRows = await fetchAllServices(state.baseDate, state.days);
    ...
    setReady();
    renderCards(); // computeGlobalSummary se llama dentro
    
    // Mostrar resumen del período cargado
    const withServices = state.empresarios.filter(e => (state.services[e.ownerCode]||[]).length > 0).length;
    toast(`✓ ${fmt(start)} → ${fmt(end)} · ${allRows.length} servicios · ${withServices} empresarios activos`);
